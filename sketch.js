/*
A game Traffic Signal created by Vibha S.

Level 1:
The game starts when we press right arrow key and when the person is crossing with the signal red..he will be givn an alert message / warning 
and if he succesfully reached the other end of the road,then he can enter to the next level.

This game also has few obstacles which the player will have to overcome.

Level 2:
The second level will start only when the person reaches the other end without touching any obstacle.

In this level...there would be more obstacles coming..so the player has to be more careful.


*Main Moto of the game*:
The main moto of my game is to create safety awareness while crossing the road.

Obstacles in my game refer to the vehicles.
*/
...
function setup() {
	...
	traffic_light = createSprite(1400,280,50,50);
	if(gameState==="start"){
	traffic_light.addImage(traffic_light_red_img);
	traffic_light.scale = 0.2;
	}

	traffic_light_stand = createSprite(1400,460,20,180);
